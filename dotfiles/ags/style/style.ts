...
const t = (dark: Opt<any> | string) => `${dark}`

// eslint-disable-next-line @typescript-eslint/no-explicit-any
const $ = (name: string, value: string | Opt<any>) => `$${name}: ${value};`

const variables = () => [
    $("bg", blur.value ? `transparentize(${t(dark.bg)}, ${blur.value / 100})` : t(dark.bg)),
    $("fg", t(dark.fg)),

    $("primary-bg", t(dark.primary.bg)),
    $("primary-fg", t(dark.primary.fg)),

    $("error-bg", t(dark.error.bg)),
    $("error-fg", t(dark.error.fg)),
    ...
    $("widget-bg", `transparentize(${t(dark.widget)}, ${widget.opacity.value / 100})`),

    $("hover-bg", `transparentize(${t(dark.widget)}, ${(widget.opacity.value * .9) / 100})`),
    $("hover-fg", `lighten(${t(dark.fg)}, 8%)`),

    $("border-width", `${border.width}px`),
    $("border-color", `transparentize(${t(dark.border)}, ${border.opacity.value / 100})`),
    $("border", "$border-width solid $border-color"),

    $("active-gradient", `linear-gradient(to right, ${t(dark.primary.bg)}, darken(${t(dark.primary.bg)}, 4%))`),
    ...
    $("popover-border-color", `transparentize(${t(dark.border)}, ${Math.max(((border.opacity.value - 1) / 100), 0)})`),
